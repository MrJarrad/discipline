...
**A `## Needed from you` row is only what he can act on right now.** The link is up and final, the decision is ready to be made — not "wait for my next message" or "once X is in" folded inside the ask itself, which is the exact confusion the operator flagged (2026-09-25: *"needed from me shouldn't include things that aren't actually ready, it's a bit confusing"*, on a row reading *"look at the panel and say yes... wait for my next message... it becomes a yes once the hover on everything clickable is in"*). Anything still being built or waiting on another step is **status prose above the heading**, never a queue row — it earns a row only once it is actually ready. A **fix** ready for his look carries one-line **Problem:** (what caused it, plain words) and **Solution:** (what fixed it) before the link, so he isn't re-deriving cause from nothing; a new feature needs no Problem line (operator, 2026-09-25: *"a really short simple explanation as to what was causing the problem and what fixed it"*).
