...
| For exact curves/durations/spring configs, see the `design-eng` skill | Reference, don't restate |
...
| Approximate durations/curves when `design-eng` has the exact value | Cite authority, don't guess |
...
**Do:** Pull the exact value from the `design-eng` skill and cite it in the "After" column.
...
For the full rule catalog and exact values — easing curves, duration budgets, spring config, gesture/drag physics, clip-path techniques — see the `design-eng` skill. This file covers only the review posture and format; it does not restate that catalog.
