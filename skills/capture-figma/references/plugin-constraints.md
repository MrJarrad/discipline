...
1. manifest `networkAccess.allowedDomains` must allowlist every POST origin the UI may hit — today `http://localhost:4411` (Mac listener) and the `https://….workers.dev` capture-ingest Worker. Figma rejects IP literals like `127.0.0.1`. The `networkAccess` object MUST include a `reasoning` field or the manifest is rejected.
