...
Vocabulary derived from Emil Kowalski's animation work — [animations.dev](https://animations.dev/) (adapted from emilkowalski/skills, MIT).

Turn a vague description of a motion or effect into the precise term, so the user knows what to ask for. This is a naming lookup, not a design or implementation tool — it pairs well with skills that design or review animations, but is self-contained on its own.
